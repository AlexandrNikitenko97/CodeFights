...
number of voters who haven't cast their vote yet, find the number of candidates who still have a chance to win the election.
...
electionsWinners(votes, k) = 2.
    ...
    remaining voters cast their votes for each of his opponents, he will still be the winner 
...
def electionsWinners(votes, k):
    newVotes = [i+k for i in votes]
    countWinenrs = 0
    maximum = max(votes)
    if len([i for i in newVotes if i == maximum]) == 1 and k == 0:
        countWinenrs = 1
    else:
        for i in range(len(votes)):
            if newVotes[i] > maximum:
                countWinenrs += 1            
    return countWinenrs
