...
so all the bots are afraid of them. That is why any room that is free or is located anywhere below a free room in the same 
...
def matrixElementsSum(matrix):
    ...
    for i in range(len(matrix)):
        for j in range(len_row):
            if matrix[i][j] != 0 and (j not in row_index):
                price.append(matrix[i][j])
            else: 
                row_index.append(j)
    return sum(price)
